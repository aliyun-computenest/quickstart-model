...
class ComfyUIWan22_14BClient:
    ...
    def generate_wan22_14b_t2v(self, prompt, neg_prompt, steps=20, cfg=3.5, width=1280, height=704, frames=81, fps=16):
        """🎬 Wan2.2 14B 文生视频生成 - 双阶段采样"""
        print("🎬 开始 Wan2.2 14B 文生视频任务...")
...
def main():
    """🚀 主函数 - 执行 Wan2.2 14B 文生视频任务"""
    client = ComfyUIWan22_14BClient()
    try:
        print(f"🎬 开始 Wan2.2 14B 文生视频任务...")
        print(f"📝 正向提示词: {PROMPT}")
        print(f"🚫 负向提示词: {NEG_PROMPT}")
        print(f"🔧 使用双阶段采样：高噪声模型 + 低噪声模型")
